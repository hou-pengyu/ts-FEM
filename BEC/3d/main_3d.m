...
t_lambda = 9.0;
% [A,B,H,D,U] = impl_GPE(nx, ny, nz, xl, xr, yl, yr, zl, zr, t_lambda);   % reference solution
...
for i=1:l
    ...
    tic;
    [mat_stif_h, mat_mass_h, mat_H_h, l_h(i,1), u_h] = impl_GPE(N1(i),N1(i),N1(i), xl, xr, yl, yr, zl, zr, t_lambda); % solutions of FEM
    time_fem(i,1) = toc;
    ...
    [mat_stif_H, mat_mass_H, mat_H_H, l_H(i,1), u_H] = impl_GPE(N2(i), N2(i), N2(i), xl, xr, yl, yr, zl, zr, t_lambda);
    time_HHH(i,1) = toc;
    
    tic % solutions of l_hHH and u_hHH
    [mat_stif_x, mat_mass_x, mat_H_x, l_x(i,1), u_x] = impl_GPE(N1(i), N2(i), N2(i), xl, xr, yl, yr, zl, zr, t_lambda);
    time_hHH(i,1) = toc;
    
    tic % solutions of l_HhH and u_HhH
    [mat_stif_y, mat_mass_y, mat_H_y, l_y(i,1), u_y] = impl_GPE(N2(i), N1(i), N2(i), xl, xr, yl, yr, zl, zr, t_lambda);
    time_HhH(i,1) = toc;
    
    tic % solutions of l_HHh and u_HHh
    [mat_stif_z, mat_mass_z, mat_H_z, l_z(i,1), u_z] = impl_GPE(N2(i), N2(i), N1(i), xl, xr, yl, yr, zl, zr, t_lambda);
    time_HHh(i,1) = toc;
...
%     end
    ...
    u_x_lin = impl_BEC_linearized(N1(i), N2(i), N2(i), xl, xr, yl, yr, zl, zr, u_hHH, l_H(i,1), t_lambda);
    time_hHH_lin(i,1) = toc;
    
    tic % solution of u^HhH
    u_y_lin = impl_BEC_linearized(N2(i), N1(i), N2(i), xl, xr, yl, yr, zl, zr, u_HhH, l_H(i,1), t_lambda);
    time_HhH_lin(i,1) = toc;
    
    tic % solution of u^HHh
    u_z_lin = impl_BEC_linearized(N2(i), N2(i), N1(i), xl, xr, yl, yr, zl, zr, u_HHh, l_H(i,1), t_lambda);
    time_HHh_lin(i,1) = toc;
...
%     end
    ...
    time_ts_lin(i,1) = time_HHH(i,1) + time_ini_lin(i,1) + time_hHH_lin(i,1) + time_HhH_lin(i,1)... 
                      + time_HHh_lin(i,1) + time_uHH_h_lin(i,1) + time_lamHH_h_lin(i,1); % time of inearized two-scale FEM
...
set(gca,'XLim',[30,80]);
set(gca,'YLim',[-1000,time_fem(l,1)+1000]);
set(gca,'FontSize',25);
