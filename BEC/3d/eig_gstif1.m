function [mat_stif,mat_mass,mat_H] = eig_gstif1(nx, ny, nz, xl, xr, yl, yr, zl, zr, nodes, elements)
...
% mat_mass = sparse((nx-1)*(ny-1)*(nz-1), (nx-1)*(ny-1)*(nz-1));
...
%                 for ni=1:8
%                     for nj=1:8
%                         el_mass(ni,nj) = el_mass(ni,nj) + sf(ni)*sf(nj)*const;     
...
%             mj = nodes(mj,4);
%             if mi~=-1 && mj~=-1
%                 mat_mass(mi,mj) = mat_mass(mi,mj) + el_mass(ni,nj);
