...
t_lambda = 0.0;
[~,B,H,D,U] = impl_oscillator(nx, ny, xl, xr, yl, yr, t_lambda);  % reference solution
...
L2_err_fem    = zeros(l,1);
L2_err_ts     = zeros(l,1);
L2_err_ts_lin = zeros(l,1);
...
for i=1:l
    ...
    tic;
    [mat_stif_h, mat_mass_h, mat_H_h, l_h(i,1), u_h] = impl_oscillator(N1(i),N1(i), xl, xr, yl, yr, t_lambda); % solutions of FEM
    time_fem(i,1) = toc;
    ...
    [mat_stif_H, mat_mass_H, mat_H_H, l_H(i,1), u_H] = impl_oscillator(N2(i), N2(i), xl, xr, yl, yr, t_lambda);
    time_HH(i,1) = toc;
    
    tic % solutions of l_hH and u_hH
    [mat_stif_x, mat_mass_x, mat_H_x, l_x(i,1), u_x] = impl_oscillator(N1(i), N2(i), xl, xr, yl, yr, t_lambda);
    time_hH(i,1) = toc;
    
    tic % solutions of l_Hh and u_Hh
    [mat_stif_y, mat_mass_y, mat_H_y, l_y(i,1), u_y] = impl_oscillator(N2(i), N1(i), xl, xr, yl, yr, t_lambda);
    time_Hh(i,1) = toc;
    ...
    time_ts(i,1)  = time_HH(i,1) + time_hH(i,1) + time_Hh(i,1) + time_uHH_h(i,1) +...
                    time_lamHH_h(i,1); % time of two-scale FEM
    ...
    u_x_lin = impl_oscillator_linearized(N1(i), N2(i), xl, xr, yl, yr, u_hH, l_H(i,1), t_lambda);
    time_hH_lin(i,1) = toc;
    
    tic % solution of u^Hh
    u_y_lin = impl_oscillator_linearized(N2(i), N1(i), xl, xr, yl, yr, u_Hh, l_H(i,1), t_lambda);
    time_Hh_lin(i,1) = toc;
...
set(gca,'XLim',[0.12,max(h)+0.02]);  
set(gca,'YLim',[2.5*10^(-2),1.1*10^(-1)]);
set(gca,'FontSize',25);
set(gca,'YTick',[10^(-1)]);
